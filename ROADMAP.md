...
- **Compound-body asteroid refactor (next priority).** Replace per-chunk welded Matter bodies with a single compound body per asteroid, splitting on chunk death. Welds oscillate even at stiffness 1 + high damping + 16 constraint iterations — compound bodies eliminate the issue entirely because there are no welds. This is how the Unity prototype's Rigidbody2D worked. ~200+ line rewrite touching asteroid.ts fracture logic, spawner, and GameScene barrier-enforcement. Ship after Phase 6 verification is clean.
