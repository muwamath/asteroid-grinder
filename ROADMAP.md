...
- **Arena overhaul.** Walls extend to the screen edges (no wasted margin). Asteroids descend through procedurally-generated pathways rather than a single straight chute — branching channels, constrictions, alternate routes. Weapon hardpoints are procedurally placed along those pathways instead of implicit-along-the-chute. Unlocks richer layouts and pairs naturally with the prestige "new zones where weapons can go" hook (§3 prestige).
