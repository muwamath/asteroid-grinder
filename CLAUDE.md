...
- **Per-body `gravityScale` is `{ x, y }`, not a scalar.** Mutate via `(body as unknown as { gravityScale: { x, y } }).gravityScale = { x: 0, y: multiplier }`. Phaser wraps most body setters but not this one. Used by the Fall Speed upgrade — alive compound asteroids set `gravityScale.y = fallSpeedMultiplier` at spawn and inherit the parent's value on split; upgrades walk `liveAsteroids` and call `setGravityMultiplier`.
...
- **Vitest** for pure logic (cost formulas, economy math, weapon catalog, upgrade appliers, gameplayState, shape generator, material ladder + distribution, asteroid graph split, save state, offline progress, cash rate, prestige state + shop catalog + effects + vault-shard award, reward formula, arena generator + validator + slot state) — lives under `src/**/*.test.ts`. 200 tests across 19 files. Run with `npm test`. **Bump the count here when you add or remove tests** — it drifts otherwise.
