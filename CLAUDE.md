...
- **Vitest** for pure logic (cost formulas, economy math, weapon catalog, upgrade appliers, gameplayState, shape generator, material ladder + distribution, asteroid graph split, save state, offline progress, cash rate, prestige state + shop catalog + effects + vault-shard award, reward formula, arena generator + validator + slot state) — lives under `src/**/*.test.ts`. 199 tests across 19 files. Run with `npm test`. **Bump the count here when you add or remove tests** — it drifts otherwise.
