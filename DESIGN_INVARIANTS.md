...
- **Grinder kills of cores do NOT award Shards.** Shards are a weapon-kill reward only. The grinder is brute-force cleanup — cores it kills drop only the flat $1 cash like any other chunk. This keeps weapons strictly more valuable than letting cores drop into the blades. Enforced in `damageLiveChunk` by the `killerType !== 'grinder'` guard.
- **Dead chunks are slippery; live chunks hinder.** Dead confetti chunks (`spawnDeadConfettiChunk` in `GameScene.ts`) carry `friction = 0.02` so they slide off slanted arena walls and reach the grinder. Live chunks keep `friction = 0.1` so they pile and hinder as gameplay. Never equalise the two — slippery live chunks defeat the "build weapons to clear the pile" loop, and sticky dead chunks perch on ledges and visually break the flow.
- **Air has no friction — this is space.** Every chunk body carries `frictionAir = 0`. Only gravity decelerates; sideways velocity from saw-launches and explosion recoils preserves until it hits a surface. Do not reintroduce air drag to "dampen" flying chunks — the vacuum feel is load-bearing for the game's identity.
...
- **Asteroid fall is gravity-driven, per-body `gravityScale.y = fallSpeedMultiplier`.** Alive compound asteroids set `gravityScale = {x:0, y: mul}` at spawn (and inherit parent's `mul` on split). Matter integrates gravity naturally — no per-tick velocity injection. `fallSpeedMultiplier` is 0.3 at L0 (sub-gravity, floaty entry) through 3.0 at L9. Upgrade propagation: `onUpgrade` walks `liveAsteroids` and calls `setGravityMultiplier(mul)` when the param changes. Dead chunks keep the default `gravityScale = 1.0`.
